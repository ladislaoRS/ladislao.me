...
export function renderSummary(summary, philosophy = []) {
  return `
    <section class="grid grid-cols-1 sm:grid-cols-[110px_1fr] gap-2 sm:gap-6 py-6 border-b border-zinc-200/80 dark:border-zinc-800 print:py-3 print:grid-cols-[90px_1fr] print:border-zinc-300">
      ...
      <div class="space-y-4">
        <p class="text-base sm:text-[17px] text-zinc-600 dark:text-zinc-300 leading-relaxed print:text-[8.5pt] print:leading-normal">
          ${summary}
        </p>
        ...
        ${philosophy.length > 0 ? `
          <div class="space-y-2.5 pt-1 print:space-y-1">
            ${philosophy.map(item => `
              <div class="text-sm sm:text-base text-zinc-600 dark:text-zinc-400 leading-relaxed print:text-[8pt]">
