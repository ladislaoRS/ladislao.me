...
export function renderExperience(experiences) {
  const experienceItems = experiences.map((exp) => {
    const achievementsHtml = exp.achievements.map(ach => `
      <li class="flex items-start gap-2.5 text-base text-zinc-700 dark:text-zinc-200 leading-relaxed print:text-[8pt] print:leading-snug">
        <span class="w-1.5 h-1.5 rounded-full bg-emerald-500/80 dark:bg-emerald-400 mt-2.5 shrink-0 print:bg-zinc-700"></span>
        <span>${ach}</span>
      </li>
    `).join('');

    const techChips = (exp.techStack || []).map(tech => `
      <span class="inline-block px-2.5 py-0.5 text-xs sm:text-sm font-mono bg-zinc-100 dark:bg-zinc-800/80 text-zinc-700 dark:text-zinc-300 rounded border border-zinc-200/60 dark:border-zinc-700/50 print:border-zinc-300 print:text-[7pt] print:px-1 print:py-0">
        ${tech}
      </span>
    `).join('');
      ...
      <article class="space-y-2 pb-8 last:pb-0 print:pb-3.5 print:break-inside-avoid">
        <div class="flex flex-col sm:flex-row sm:items-baseline justify-between gap-1">
          <div>
            ...
            <div class="text-sm font-mono text-zinc-500 dark:text-zinc-400 mt-0.5 print:text-[7.5pt]">
              ${exp.location}
            </div>
          </div>
          <span class="text-sm font-mono text-zinc-500 dark:text-zinc-400 shrink-0 sm:text-right print:text-[8pt] print:font-semibold">
            ${exp.period}
          </span>
        </div>

        ${exp.description ? `
          <p class="text-base text-zinc-600 dark:text-zinc-300 leading-relaxed print:text-[8pt] print:leading-snug">
            ${exp.description}
          </p>
        ` : ''}

        <ul class="space-y-2 pt-1 print:space-y-1">
          ${achievementsHtml}
        </ul>

        ${techChips ? `
          <div class="pt-2 flex flex-wrap gap-1.5 sm:gap-2 print:hidden">
            ${techChips}
          </div>
        ` : ''}
      </article>
