...
export function renderSkills(competencies) {
  const categoryBlocks = Object.entries(competencies).map(([category, skills]) => {
    const skillBadges = skills.map(skill => `
      <span class="inline-flex items-center px-2.5 py-0.5 rounded text-xs font-mono bg-zinc-100 dark:bg-zinc-800/70 text-zinc-700 dark:text-zinc-300 border border-zinc-200/60 dark:border-zinc-700/60 print:border-zinc-300 print:bg-transparent print:text-zinc-800 print:text-[7.5pt] print:py-0 print:px-1">
        ${skill}
      </span>
    `).join('');
    ...
    return `
      <div class="space-y-1.5 print-avoid-break">
        <h3 class="text-xs font-mono font-medium text-zinc-400 dark:text-zinc-500 print:text-[8pt] print:text-zinc-700">
          ${category}
        </h3>
