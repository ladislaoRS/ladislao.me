...
export function renderEducation(education = [], certifications = []) {
  const educationItems = education.map(edu => `
    <div class="space-y-1 print-avoid-break">
      <div class="flex flex-col sm:flex-row sm:items-baseline justify-between gap-1">
        ...
        <span class="text-xs sm:text-sm font-mono text-zinc-400 dark:text-zinc-500 shrink-0 sm:text-right print:text-[8pt]">
          ${edu.period}
        </span>
      </div>
      <div class="text-xs sm:text-sm font-mono text-zinc-500 dark:text-zinc-400 print:text-[8pt]">
        ${edu.institution}
      </div>
      ${edu.details ? `
        <p class="text-sm sm:text-base text-zinc-500 dark:text-zinc-400 leading-relaxed pt-0.5 print:text-[7.5pt]">
          ${edu.details}
        </p>
      ` : ''}
    </div>
  ...
  const certItems = certifications.map(cert => `
    <div class="flex items-baseline justify-between gap-3 text-sm sm:text-base py-1.5 print:py-0.5 print-avoid-break">
      <div>
        <span class="font-medium text-zinc-900 dark:text-zinc-100 print:text-[8pt]">${cert.title}</span>
        <span class="text-zinc-400 dark:text-zinc-500 text-xs sm:text-sm font-mono print:text-[7.5pt]"> &bull; ${cert.issuer}</span>
      </div>
      <span class="text-zinc-400 dark:text-zinc-500 text-xs sm:text-sm font-mono shrink-0 print:text-[7.5pt]">${cert.year}</span>
    </div>
  `).join('');
      ...
      <div class="space-y-6 print:space-y-3">
        ...
        ${certifications.length > 0 ? `
          <div class="pt-4 border-t border-zinc-200/60 dark:border-zinc-800/60 print:border-zinc-300">
            <h3 class="text-xs sm:text-sm font-mono font-medium text-zinc-400 dark:text-zinc-500 mb-2 print:text-[8pt] print:text-zinc-700">
